...
const DEFAULT_TEXT = `Слепая печать — это метод набора текста на клавиатуре, при котором человек не смотрит на клавиши. Этот навык позволяет значительно увеличить скорость набора и снизить количество ошибок. Для освоения слепой печати требуется регулярная практика и терпение.`

function TrainTextPage() {
  ...
  // Получение своих текстов типа "text"
  const { data: myTexts } = useQuery({
    queryKey: ['texts', 'my', 'text'],
    queryFn: () => textsApi.getMy({ type: 'text' }),
  })

  // Получение публичных текстов типа "text"
  const { data: publicTexts } = useQuery({
    queryKey: ['texts', 'public', 'text'],
    queryFn: () => textsApi.getPublic({ type: 'text' }),
  })
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <h1 className="mb-4 text-3xl font-bold">Тренировка: Текст</h1>
